...
Include the actual code the module will use in code↔English translation blocks. Copy-paste from the codebase with file path and line numbers. The writing subagent will use these verbatim — it will NOT re-read the codebase.
...
Check which elements this module needs. Include enough detail for the writing subagent to build them.
...
List only the sections the writing subagent needs — not the whole file.

- `references/interactive-elements.md` → [section names, e.g., "Group Chat Animation", "Interactive Architecture Diagram"]
